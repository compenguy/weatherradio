...
impl std::fmt::Display for Measured {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "[{}][{:04x}/{:02x}] {}", self.timestamp, self.device_id.unwrap_or(0x0000), self.channel.unwrap_or(0x00), self.measurement)
    }
}
...
impl ToMeasurements for &[u8] {
    fn to_measurements(&self) -> Result<Vec<Measured>, SensorError> {
        if let Some(payload) = bitstream_to_payload(self) {
            ...
            let measurements = match payload[0] {
...
impl ToMeasurements for Ws68 {
    fn to_measurements(&self) -> Result<Vec<Measured>, SensorError> {
        ...
        measurements.push(tmp.clone());
        tmp.measurement =
            Measurement::WindSpeed(Velocity::new::<velocity::kilometer_per_hour>(self.w_speed as u16));
        measurements.push(tmp.clone());
        tmp.measurement =
            Measurement::WindGust(Velocity::new::<velocity::kilometer_per_hour>(self.w_gust as u16));
        measurements.push(tmp.clone());
...
mod test {
    ...
    const WH31_E_PAYLOADS: [[u8; 11]; 11] = [
    ...
    const WH31_E_RCC_PAYLOADS: [[u8; 11]; 14] = [
    ...
    fn test_wh31_e() {
        ...
        for (i, row) in WH31_E_PAYLOADS.iter().enumerate() {
            ...
            let payload = bitstream_to_payload(&row_vec).expect("Found no payload in the bitstream");
            assert_eq!(&payload[0..payload.len()-4], row);

            let measurements = row_vec.as_slice().to_measurements().expect("Failed to convert payload data to content");
    ...
    fn test_wh31_e_rcc() {
        // Test increasing amounts of sync values at the beginning of the payload
        for (i, row) in WH31_E_RCC_PAYLOADS.iter().enumerate() {
            let mut row_vec: Vec<u8> = vec![0xaa; i];
            row_vec.extend_from_slice(&PREAMBLE);
            row_vec.extend_from_slice(row);
            row_vec.extend_from_slice(&[0x00; 4]);
    ...
    fn test_wh40() {
        // Test increasing amounts of sync values at the beginning of the payload
        for (i, row) in WH40_PAYLOADS.iter().enumerate() {
            let mut row_vec: Vec<u8> = vec![0xaa; i];
            row_vec.extend_from_slice(&PREAMBLE);
            row_vec.extend_from_slice(row);
            row_vec.extend_from_slice(&[0x00; 4]);
    ...
    fn test_ws68() {
        // Test increasing amounts of sync values at the beginning of the payload
        for (i, row) in WS68_PAYLOADS.iter().enumerate() {
            let mut row_vec: Vec<u8> = vec![0xaa; i];
            row_vec.extend_from_slice(&PREAMBLE);
            row_vec.extend_from_slice(row);
            row_vec.extend_from_slice(&[0x00; 4]);
